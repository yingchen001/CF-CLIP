...
from criteria.vgg_loss import VGGLosses_fix
...
class Coach:
	def __init__(self, opts):
		...
		self.source_text = self.opts.source_text 
		...
		self.n_latent = self.net.decoder.n_latent
		self.clip_feat = clip_loss.CLIPText(opts)
		# Initialize loss
		if self.opts.id_lambda > 0:
			self.id_loss = id_loss.IDLoss(self.opts).to(self.device).eval()
		if self.opts.perc_lambda > 0:
			self.vgg_loss = VGGLosses_fix(opts)
		if self.opts.latent_l2_lambda > 0:
			self.latent_l2_loss = nn.MSELoss().to(self.device).eval()

		self.nce_loss = nce_loss.NCELoss(self.device, clip_model="ViT-B/32")
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			for batch_idx, batch in enumerate(self.train_dataloader):
				...
				t_feat = self.clip_feat(self.text_inputs).to(self.device).detach()
				t_feat = t_feat.unsqueeze(1).repeat(batch.shape[0],18,1).float()
				t_w = self.net.text_mapper(t_feat)
				if not self.opts.w_space:
					w = batch
				else:
					w = batch.unsqueeze(1).repeat(1,self.n_latent,1).float()
				w = w.to(self.device)
				with torch.no_grad():
					x, _ = self.net.decoder([w], input_is_latent=True, randomize_noise=False, truncation=1)
				
				w_hat = w + 0.1 * self.net.mapper(torch.cat([w, t_w], dim=-1))
	...
	def validate(self):
		...
		for batch_idx, batch in enumerate(self.test_dataloader):
			if batch_idx > 200:
				break
			t_feat = self.clip_feat(self.text_inputs).to(self.device).detach()
			t_feat = t_feat.unsqueeze(1).repeat(1,18,1).float()
			t_w = self.net.text_mapper(t_feat)
			if not self.opts.w_space:
				w = batch
			else:
	...
	def configure_optimizers(self):
		...
		params += list(self.net.text_mapper.parameters())
	...
	def calc_loss(self, w, x, w_hat, x_hat):
		...
		if self.opts.perc_lambda > 0:
			loss_vgg, _, _ = self.vgg_loss(x_hat, x)
			loss_dict['loss_perc'] = float(loss_vgg)
			loss += loss_vgg * self.opts.perc_lambda
		if self.opts.latent_l2_lambda > 0:
		...
		x_aug = torch.cat([self.augment(x_hat) for i in range(self.num_aug)], dim=0)
		x = x.repeat(self.num_aug,1,1,1)
		loss_nce = self.nce_loss(x, self.source_text, x_aug, self.opts.description).sum()
		loss_dict['loss_nce'] = float(loss_nce)
